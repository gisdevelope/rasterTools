...
#     
#     productFiles <- downloadMODIS(getFiles = paste0(rtPaths$modis$online, satellite, "/", product, ".", productVersion, "/", validDates[i], "/"))
#     
#     modis_out <- list()
#     # go through all files, which are available according to the arguments, for the tile
#     allObjects <- NULL
#     
#     for(j in seq_along(unique(tabTiles$id))){
#       
#       tileID <- unique(tabTiles$id)[j]
#       
#       # determine gridId to narrow down the files
#       colRowNames <- which(nameMat==tileID, arr.ind = T)-1
#       gridId <- paste0(c("h", "v"), sprintf('%02i', as.integer(rev(colRowNames))), collapse = "")
...
#       } else{
#         
#         # To process the tile, carry out the following four steps, probably can
#         # all happen without having to load the data into R.
#         # 1) Check quality bits and assign data to a (temporary) GeoTIFF
#         if(!raw){
#           # this must happen in c++, think of a solution how to manage that smartly, because every product has the QC bit at another layer
#           # it looks as if in the end I don't even need a meta_modis file anymore, as these information would be "hardcoded" into the c++ function
#           for(k in 1:dim(tempObject)[3]){
#             oldVals <- values(tempObject[[k]])
#             newVals <- oldVals * meta$correction_factor[k]
#             newVals[newVals < meta$valid_min[k] | newVals > meta$valid_max[k]] <- NA
...
#     }
#     
#     # after loading all tiles per date, put together the mosaic (should be done
...
#     
#     
#     # Only now load the produced GeoTiff into R.
#     
#     # modis_out <- c(modis_out, setNames(list(allObjects), gridId))
#   }
#   
