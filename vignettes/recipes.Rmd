...
link-citations: yes
vignette: >
  %\VignetteIndexEntry{Complex spatial operations}
...
In this document you can find a collection of spatial operations in the form of algorithms or more complex combinations of the functions provided in `rasterTools`. It is assumed that you have read the [introduction](/rastertools/articles/terminology.html) and understand what *operator* and *algorithm* means in the scope of `rasterTools`.
...
Moreover, we need to create two frequently occuring derivations thereof, a raster where classes are assigned to distinct patches and a binarised raster, where some class(es) have been set as focal class. Here we derive them from the categorial raster, but this could as well be done from the continuous raster.
...
We may be interested in openings (or perforations, as they are called elsewhere) of foreground patches out of several reasons. Hence, we either want to end up with a raster of the openings without anything else, or a raster where the openings are labeled in a systematic way.
...
We see that there are several patches that haven an opening, but there is also an "opening" between the large lower-left turquoise patch and its tiny neigbour to the bottom. In some situations this tiny neighbour might a full-grown patch which we actually have to consider for ecological inference. To avoid such problems, we should segregate the raster into patches and carry out our identification algorithm on these.
...
## Identify the distance map of centroids per patch
